...
export function ProductTable() {
  ...
  const deleteProduct = (productIdDeleting: number) => {
    setFilteredProducts((prev) => prev.filter((product) => product.id !== productIdDeleting))
  }
        ...
        <table className="my-[-20px] w-full border-separate border-spacing-y-5 gap-4 text-center text-sm">
          <thead className="sticky top-0">
            <tr className="bg-gray-100">
              <th className="rounded-l-md border-t border-b border-l border-gray-300 p-2 px-5">ID</th>
              ...
              <th className="rounded-r-md border-t border-r border-b border-gray-300 p-2 px-5">Actions</th>
            </tr>
          </thead>
          ...
          <tbody>
            {loading ? (
              ...
              filteredProducts.map((product) => (
                <tr key={product.id}>
                  <td className="rounded-l-md border-t border-b border-l border-gray-300 p-2 px-5">{product.id}</td>
                  ...
                  <td className="border-t border-b border-gray-300 p-2 px-5">{product.description}</td>
                  <td className="border-t border-b border-gray-300 p-2 px-5">${product.price.toFixed(2)}</td>
